...
{
    public partial class frmMBTRentalForm : Form
    {
        int iRowIndex;
        ...
        string constr = "Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" + Application.StartupPath + @"\MBTRentalDB.mdb";
        int nmonth;
        int uid;
        DateTime date1, date2;

        public frmMBTRentalForm()
        {
            InitializeComponent();
        }
        ...
        void databind()
        {
            ...
            while (myreader.Read())
            {
                ...
                item_price = double.Parse(myreader.GetValue(7).ToString());
          ...
          //  item.ItemName = item_name;
            ...
            double total = 0;
            foreach (double price in lstPrice.Items)
            {
                total += price;
            }
        ...
        {
            DBClass db = new DBClass();
            string dimensions = txtDimensionKey.Text;
            string color = txtColorKey.Text;
            string rating = txtRateKey.Text;
            string monthlyFee = txtMonthlyKey.Text;
            string capacity = txtCapcityKeyword.Text;

            // Construct the search query with parameters
            string searchQuery = "SELECT Item.*, Item.Capacity " +
                                 "FROM Item " +
                                 "WHERE Item.Capacity LIKE @Capacity " +
                                 "AND Item.available = 'yes' " +
                                 "AND Item.Dimensions LIKE @Dimensions " +
                                 "AND Item.Colour LIKE @Colour " +
                                 "AND Item.[Monthly Fee] LIKE @MonthlyFee " +
                                 "AND Item.Rating LIKE @Rating " +
                                 "ORDER BY Item.id";

            OleDbConnection connection = new OleDbConnection(constr);
            OleDbDataAdapter dataadapter = new OleDbDataAdapter();
            dataadapter.SelectCommand = new OleDbCommand(searchQuery, connection);

            // Add parameters and set their values
            dataadapter.SelectCommand.Parameters.AddWithValue("@Capacity", $"%{capacity}%");
            dataadapter.SelectCommand.Parameters.AddWithValue("@Dimensions", $"%{dimensions}%");
            dataadapter.SelectCommand.Parameters.AddWithValue("@Colour", $"%{color}%");
            dataadapter.SelectCommand.Parameters.AddWithValue("@MonthlyFee", $"%{monthlyFee}%");
        ...
        private void lstItemName_SelectedIndexChanged(object sender, EventArgs e)
        {
